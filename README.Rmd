...
This repo contains a Shiny application that shows data from OECD concerning graduates and population per Country and Year.
...
The dataset on graduates counts  ISCED starting from level 3, which is upper seconddary education.

The dataset on population contains a mid-year population data for the 34 OECD member countries. 

The datasets are subset to contain the last five years of data, resulting in years 2008 until 2012. Since the graduates did not contain data on all of the 34 OECD member countries, the datasets are merged to keep only the countries in common.

This repo contains two files for the datasets and three files for the source code. The two files for the dataset are copied in the repo and available in the OECD pages listed above. The three files for the source code are:

- global.R - reads and manipulates the data so it is accessible for both ui.R and server.R
...
- server.R - Present the two charts for all the years at first. Then, dynamically update the two charts based on the user selection of the year.
